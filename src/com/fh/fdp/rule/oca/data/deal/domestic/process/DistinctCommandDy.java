...
public class DistinctCommandDy extends RuleBaseCommand {
	...
	private String app_hardware_feature_key = Constant.APP_HARDWARE_FEATURE_KEY;
	...
	public void execute(Message msg) throws Exception {
		...
		for (List<DataField> row : inputRows) {
			for (DataField field : row) {
				if (Constant.APP_HARDWARE_FEATURE.equalsIgnoreCase(field.getName())) {
					...
					for(DataField d : data){
						if (Constant.isAppType(d.getName())) {
							String key = d.getValue() + app_hardware_feature_key + field.getValue();
