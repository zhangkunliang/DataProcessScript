                    ...
                    data.forEach(d -> {
                        if (Constant.isAppType(d.getName())) {
                            synchronized (JedisUtil.class) {
                                boolean isExitId = jedis.exist(d.getValue() + Constant.TASK_RELATION_KEY + field.getValue(),
                                        config.getAtvtCacheRelation());
                                if (!isExitId) {
                                    jedis.add(d.getValue() + Constant.TASK_RELATION_KEY + field.getValue(),
                                            String.valueOf(System.currentTimeMillis() / 1000), expandTime,
                                            config.getAtvtCacheRelation());
                                    if (!cache.containsKey(d.getValue() + Constant.TASK_RELATION_KEY + field.getValue())) {
                                        outputRowsDistinct.add(row);
                                    }
                                    cache.put(d.getValue() + Constant.TASK_RELATION_KEY + field.getValue(),
